...
*(No active chores at this time.)*

---

## ✅ Completed Chores

* [x] **`ENTROPY-01` Configurable Plaintext Entropy Source URLs (`[entropy] source_urls`)**: Allowed operators to configure arbitrary plaintext URLs in `randbotd.toml` (`[entropy] source_urls = [...]`) or via `--entropy-source-urls` CLI flag as dynamic Gutenberg drilldown alternatives. The engine fetches text content (HTML, CSV, TXT, JSON — strictly excluding binary/PDF/multipart/`application/octet-stream`), splits words across non-alphabetic boundaries, deduplicates the corpus vocabulary, computes true Shannon entropy ($H = \log_2(N)$), and samples CSPRNG words across iterations until reaching 256 bits.
* [x] **`CA-ID-01` Bind `ca_id` to Owner Node Pubkey (composite)**: Updated `compute_ca_id` in [`src/pki/ca.rs`](file:///home/mreugenej7/git/randbotd/src/pki/ca.rs) to derive CA IDs deterministically via `SHA-256("randbotd_v1_ca_identity_domain:" || common_name || ":" || node_pubkey_bytes)`.
