...
* **Opt-In Risk Floors (Downward-Only Risk Expansion)**: CA operators retain sovereignty to adjust their accepted risk floor downward below their dynamic lower confidence bound ($50\% - \Delta_{\text{CA}}$), enabling courageous nodes to host high-risk, censorship-resistant, or dissident domains while taking accountability for their aggregate CA reputation. Raising a risk floor above lower $\Delta$ to cherry-pick top-reputation domains ("domain-fishing fraud") is strictly prohibited and rejected by fair-band matching.
...
* **Critical Custom OID Extension Enclosure & Voluntary Opt-In**: Every native `randbotd` certificate includes a custom X.509 extension marked `critical = TRUE` (`OID 2.25.332006307751889903095271628869501346770.1.1`, derived from ITU-T X.667 decentralized UUID `f9c616c7-8e4d-4f84-a32e-596b5ada63d2`). Per RFC 5280, un-augmented legacy browsers automatically reject the certificate. This guarantees **voluntary participation and prevents free-riding**: `randbotd` never forces itself onto un-consenting browsers or default OS trust stores. Using our Web-of-Trust verification requires an explicit opt-in choice by installing a companion extension or running a local proxy daemon. The root UUID provenance signature is committed in [`docs/genesis_oid.asc`](file:///home/mreugenej7/git/randbotd/docs/genesis_oid.asc); to verify this signature, fetch the author's public key from **The Random Consortium landing page** (`the.consortium.randºm`). Furthermore, it renders stolen or fake-payment certs completely useless: inside the network, they are blocked by P2P `TxKeyProof` consensus; outside the network, they are rejected by standard TLS parsers due to the unrecognized critical OID.
* **Protocol Blindness & Client Delegation Sovereignty**: `randbotd` operates as a pure, agnostic consensus engine—*we provide the mathematics, the Web-of-Trust evaluation, and the P2P game theory*. Extension and client proxy developers retain full sovereignty: they can choose to inspect the critical OID in client code for fast-path filtering, or relegate 100% of certificate evaluation, `out-of-net` tagging, and trust verification directly to the daemon API (`ECO-04`). The protocol imposes no rigid client-side enforcement rules.
...
* **In-Bot Monero Wallet & 3-Step Escrow-Less Settlement**: Automated P2P fee settlement directly between in-bot Monero addresses using cryptographic proof-of-issuance smart contracts. Enforces a 3-step state machine: 1) **Constitution** (accorded fee $X$, CA recipient address, and cert parameters), 2) **Cert Emission** (CA signed certificate matching terms), and 3) **TxKey Proof** (domain owner Monero transaction secret key `tx_key` proving on-chain payment of $X$). Third-party P2P nodes validate all 3 steps independently before admitting the transaction into global WoT consensus. Any rogue client or CA modifying local binary code to fake payment or issuance lives in an isolated **solipsistic parallel universe**—their local node accepts the lie, but no other node on the network recognizes the cert or transaction.
