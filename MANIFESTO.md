...
* **Critical Custom OID Extension Enclosure**: Every native `randbotd` certificate includes a custom X.509 extension marked `critical = TRUE` (`OID 2.25.332006307751889903095271628869501346770.1.1`, derived from ITU-T X.667 decentralized UUID `f9c616c7-8e4d-4f84-a32e-596b5ada63d2`). Per RFC 5280, un-augmented legacy browsers automatically reject the certificate, ensuring native certificates cannot leak or be misused outside the `randbotd` ecosystem without requiring any central IANA form or identity registration. The root UUID provenance signature is committed in [`docs/genesis_oid.asc`](file:///home/mreugenej7/git/randbotd/docs/genesis_oid.asc); to verify this signature, fetch the author's public key from **The Random Consortium landing page** (`the.consortium.randºm`). Browser extensions and proxy plugins leverage this critical OID to route validation directly to the local P2P daemon. Furthermore, it renders stolen or fake-payment certs completely useless: inside the network, they are blocked by P2P `TxKeyProof` consensus; outside the network, they are rejected by standard TLS parsers due to the unrecognized critical OID.
