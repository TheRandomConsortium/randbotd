...
| `CA-02` | **Cryptographic Agility Suite** | Multi-algorithm key generation and certificate signing: RSA-4096, ECDSA P-384, Ed25519, and ML-DSA-44 (NIST FIPS 204 Dilithium2 PQC) with Argon2id + ChaCha20-Poly1305 masterpass key encryption. | 🟢 |
...
| `CA-11` | **Distributed Custodian Swarm & Threshold Key Delegation Engine** | Enables CAs to distribute operational load across $n$ decentralized nodes using threshold cryptography (FROST / $m$-of-$n$ Schnorr/Ed25519 DKG). Governed by a 4-step escrow-less delegation state machine (`CustodianContract` $\rightarrow$ `CustodianDelegationRequest` $\rightarrow$ `CACapabilitiesProof` dummy build $\rightarrow$ `SwarmActivationConfirmation`) with zero upfront fees, settling worker compensation via trustless per-issuance **Pay-As-You-Work Revenue-Share** (`PAY-03` Monero split to active $m$ co-signers). Note: Premature domain purges (`PAY-06`) require splitting prorated refund costs among revenue-earning swarm nodes, retaining an underlying operational trust requirement among custodians. | ⚪ |
| `CA-12` | **Multi-Tier CA Certificate Offer Catalog & Profile Engine** | Replaces single static pricing with a structured Offer Catalog (`CAPublishOfferCatalog`). Allows CAs to publish multiple `CertificateProfile` / `CertificateOffer` options (e.g. Standard Clearnet, Tor, Micro-TTL, Multi-Network) seamlessly integrated with CA identity and local storage subtables. | 🟢 |
...
| **Phase 3** | `CA-04`, `CA-07`, `CA-09`, `CA-11` | P2P broadcast, bad-domain purge, key rotation, and distributed custodian threshold swarms (`CA-11`) all depend on valid certs existing and cert builder being operational. Note on `CA-11`: Launches as a **non-monetary load-distribution swarm** (0-cost high availability) in Phase 3; monetized Pay-As-You-Work revenue-share (`PAY-03`) and purge refund liabilities (`PAY-06`) activate during **Global Phase 9** alongside Monero market integration. |
...
| `ACME-06` | **Emergency Default Fallback CA** | Provisions temporary, short-TTL certificates if a randomly matched P2P CA is offline or unresponsive, ensuring zero downtime while waiting for full issuance. If a CA fails to answer or proxy outages (`ProofError::BackendUnreachable`) occur, the client generates a short-lived emergency certificate (24h TTL). Upon TTL expiration, the client checks P2P events for the issuance response; if still unanswered after `max_emergency_short_certs` iterations, the client re-enters the fair-band matching pool. | 🔴 |
...
| `PAY-07` | **Domain Purge Bounty Engine** | Solves liquidity bottlenecks and non-paying custodian workers during domain purges (`CA-07`/`PAY-06`). When a CA needs to purge a domain but lacks liquidity or faces a non-paying custodian, it emits a P2P `DomainPurgeBounty` with an agreed yield rate ($\Delta r$). Any market participant (bounty hunter) funds the prorated refund **directly to the domain owner's address** (`BountyFulfillmentProof`). Because the CA receives 0 XMR directly, **CA exit-scams and moral hazard are eliminated**. Future certificate issuance revenue earned by that CA under `PAY-03` is routed to repay the bounty hunter (Principal + Interest) first before net CA revenue. Bounty hunters make informed risk decisions by evaluating the CA's historical issuance volume and WoT reputation. | 🔴 |
